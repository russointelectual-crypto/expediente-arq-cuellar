 ...
 *
 * Versión con OBSERVER: la orden es el SUJETO. Cada vez que cambia de estado (y cuando
 * la revisión diaria detecta que se acerca el plazo de 6 meses) publica un EventoOrden
 * a todos sus observadores. La orden no sabe quién escucha ni qué hace cada uno.
 */
final class OrdenDeTrabajo
{
    ...
    // =====================================================================
    //  Cobro — versión base, SIN patrón: if/else según cómo terminó la orden
    // =====================================================================

    public function totalACobrar(): float
    {
        if ($this->fueSolucionada()) {
            return $this->manoDeObra + $this->totalRepuestos();
        }
        if ($this->motivoSinSolucion === self::CLIENTE_NO_AUTORIZO && $this->tipoTrabajo === self::TRABAJO_ELECTRONICA) {
            return $this->costoDiagnostico;                       // RN7
        }
        return 0.0;
    }
